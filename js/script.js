window.addEventListener("load", function () {
  ...
  xhttp.onreadystatechange = function (e) {
    ...
    if (req.readyState === XMLHttpRequest.DONE) {
      ...
      showReviewArr(); // 리뷰를 화면에 배치
      showNotice(); // 공지사항을 화면에 배치
      showGoodnews(); // 물품소식을 화면에 배치
    }
  };
  // 자료를 호출한다.
  ...
  function showVisual() {
    ...
    swVisualPlay.addEventListener("click", function () {
      // 현재 active 클래스가 있는지 확인하고 기능을 설정
      // 만약 classList에 contains가 active면
...
<a href="${obj.link}" class="new-txt">
  ...
  function showPopularIcon() {
    ...
    tag.forEach(function (item, index) {
      ...
      // 아이콘에 클릭하면 버튼 (.popular-more)의 글자를 클릭된 타이틀의 글자로 변경
  ...
  function showBuyGood() {
    ...
    const chkArr = document.querySelectorAll(".season-item");
    chkArr.forEach(function (item) {
      const state = item.checked; // 현재 체크박스에 체크 상태 확인
      if (state) {
        // 체크가 되어 있다면
        count += 1; // 체크된 상품의 수를 증가 count++
        const price = parseInt(item.value); // 체크된 상품의 가격을 정수로 변환하여 가져옴
        priceTotal += price;
      }
    });
    buyTotalCount = count; // 체크된 상품의 수를 전역변수에 저장
    buyTotalMoneyPrice = priceTotal; // 총 가격을 전역변수에 저장
